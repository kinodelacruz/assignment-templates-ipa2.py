...
def shift_letter(letter, shift):
    ...
    *Note: the single underscore `_` is used to acknowledge the presence
    ...
    if letter == " ":
        ...
        result = "Please enter an uppercase letter or a space."
        return result
...
def vigenere_cipher(message, key):
    ...
    for character in message:
        if character == " ":
            ...
            character2 = key[cnt]
            new_ord = ord(character) + (ord(character2)-65)
            if new_ord > ord("Z"):
               new_ord = new_ord - 26
            result = chr(new_ord)
            new_message.append(result)
...
def scytale_cipher(message, shift):
    ...
    for i in range(0, len(message)):
        m_index = (i // shift) + (len(message) // shift) * (i % shift)
        result = message[m_index]
...
def scytale_decipher(message, shift):
    ...
    # Replace `pass` with your code.
    # Stay within the function. Only use the parameters as input. The function should return your answer
    num_columns = int(len(message) / shift)
    num_rows = shift
    index = 0
    decoded_message = [""] * num_rows
    
    for column in range(num_columns):
        for row in range(num_rows):
            decoded_message[row] += message[index]
            index += 1
